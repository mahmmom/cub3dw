...
/*   Updated: 2024/04/09 17:55:51 by mohamoha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (is_valid_pos_wall_collision(data, x, y) && is_valid_pos_in_map(data, x, y))
		return (true);
	return (false);
}
...
int validate_move(t_data *data, double new_x, double new_y)
{
    ...
    {
        data->p.pos_x = new_x; // Update the player's position only if the move is valid
        data->p.pos_y = new_y;
        moved = 1;
    }
    return moved;
