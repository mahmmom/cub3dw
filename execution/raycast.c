...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/08 20:45:18 by mohamoha          #+#    #+#             */
/*   Updated: 2024/04/09 16:17:51 by mohamoha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static void	init_raycasting_info(int x, t_ray *ray, t_p *player)
{
	init_ray(ray);
...
static void	set_dda(t_ray *ray, t_p *player)
{
	if (ray->dir_x < 0)
	{
		ray->step_x = -1;
...
static void	perform_dda(t_data *data, t_ray *ray)
{
	...
	while (hit == 0)
	{
		...
		if (ray->map_y < 0.25
			...
			break ;
		else if (data->map.map_data[ray->map_y][ray->map_x] > '0')
			hit = 1;
	}
}

static void	calculate_line_height(t_ray *ray, t_data *data, t_p *player)
{
	if (ray->side == 0)
